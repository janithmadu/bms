...
export async function PUT(
  ...
  try {
    ...
    } else if (existingBooking.status === 'confirmed' && status === 'cancelled') {
      // Refund tokens when cancelling a confirmed booking
      tokenAdjustment = existingBooking.tokensUsed
    } else if (existingBooking.status === 'cancelled' && status === 'confirmed') {
      // Deduct tokens when confirming a previously cancelled booking
      const tokenRecord = await prisma.token.findUnique({
        where: { id: 'singleton' }
      })
      
      if (!tokenRecord || tokenRecord.availableCount < existingBooking.tokensUsed) {
    ...
    const result = await prisma.$transaction([
      ...
      ...(tokenAdjustment !== 0 ? [
        prisma.token.update({
          where: { id: 'singleton' },
          data: {
            availableCount: { increment: tokenAdjustment },
            tokensUsedThisMonth: { increment: -tokenAdjustment }
          }
        })
      ] : [])
    ])
